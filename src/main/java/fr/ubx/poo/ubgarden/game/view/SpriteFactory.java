...
public final class SpriteFactory {
    ...
    public static Sprite create(Pane layer, GameObject gameObject) {
        ...
        if (gameObject instanceof Door door) {
            ImageResource doorImage = door.isOpen() ? DOOR_OPENED : DOOR_CLOSED;
            return new Sprite(layer, factory.get(doorImage), door) {
                @Override
                public void updateImage() {
                    Door d = (Door) getGameObject();
                    ImageResource img = d.isOpen() ? DOOR_OPENED : DOOR_CLOSED;
                    setImage(factory.get(img));
                }
            };
        }
        ...
        // characters
        if (gameObject instanceof Gardener)
            return new SpriteGardener(layer, (Gardener) gameObject);
