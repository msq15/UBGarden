...
public class StatusBar {
    ...
    public StatusBar(Group root, int sceneWidth, int sceneHeight) {
        ...
        this.mainLayout.getChildren().addAll(level, status);
        this.mainLayout.getStyleClass().add("statusBar");
        this.mainLayout.relocate(0, sceneHeight);
        this.mainLayout.setPrefSize(sceneWidth, height);
        root.getChildren().add(this.mainLayout);
    }
    ...
    private void updateLevel(int n) {
        if (n != gameLevel) {
            ...
            ImageView levelDigit = new ImageView(ImageResourceFactory.getInstance().getDigit(n));
    ...
    public void update(Game game) {
        ...
        insecticideNumber.setText(Integer.toString(game.getGardener().getInsecticideCount()));
